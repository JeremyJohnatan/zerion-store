...
export default async function KatalogAppsPage() {
  let appServices: any[] = [];

  try {
    appServices = await prisma.service.findMany({
      where: { type: "APP_PREMIUM" },
    ...
    <main className="flex-1 w-full bg-zinc-50 min-h-screen pt-12 md:pt-20 pb-20">
      <div className="max-w-7xl mx-auto px-8 mb-12">
        <h1 className="text-4xl md:text-5xl font-black uppercase text-black mb-4">
          Semua <span className="text-[#c8ff00] drop-shadow-[2px_2px_0px_rgba(0,0,0,1)]">App Premium</span>
        </h1>
        <p className="text-lg text-zinc-500 font-medium max-w-2xl">
          Berbagai layanan aplikasi premium original dengan harga terbaik. Resmi dan bergaransi.
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8">
        ...
        <div className="mt-20 bg-white border-4 border-black p-8 md:p-12 text-center rounded-sm shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden">
          ...
          <h2 className="text-2xl md:text-3xl font-black uppercase text-black mb-4 relative z-10">Aplikasi yang Dicari Tidak Ada?</h2>
          <p className="text-zinc-600 font-medium mb-8 max-w-xl mx-auto relative z-10">
            Aplikasi Premium yang Anda inginkan belum masuk katalog? Hubungi kami, dan kami akan usahakan mendapatkannya dengan harga termurah!
          </p>
          <RequestChatButton defaultMessage="Halo Admin, saya mau request Aplikasi Premium yang tidak ada di katalog." />
        </div>
      </div>
    </main>
  );
