...
export default function EditServicePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    fetch(`/api/services/${id}`)
      ...
      .then(result => {
        if (result.success) {
          ...
          setError(result.error || "Gagal memuat data layanan.");
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let imageUrl = imagePreview; // Default to existing image if unchanged
      ...
      const res = await fetch(`/api/services/${id}`, {
        ...
        body: JSON.stringify({
          ...
          imageUrl: imageUrl.startsWith('blob:') ? '' : imageUrl, // Only save if it's a real URL
        }),
      });
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black uppercase text-black">Edit Layanan</h1>
          <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest mt-1">Ubah detail layanan joki</p>
        </div>
      </div>
      ...
      <div className="bg-white border-2 border-black rounded-md p-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-black mb-2">Upload Foto Thumbnail (Opsional)</label>
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-[#bbf7d0] border-4 border-black p-8 rounded-sm shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-center animate-in fade-in zoom-in duration-300 max-w-sm w-full mx-4">
            ...
            <p className="text-sm font-bold text-black uppercase tracking-widest mb-6">Data layanan telah diperbarui</p>
            <BrutalistButton 
              variant="secondary" 
              onClick={() => {
